...
test("hero uses Space Grotesk with a Departure Mono pixel accent", () => {
  ...
  assert.ok(pkg.dependencies["@fontsource-variable/space-grotesk"]);
  assert.equal(pkg.dependencies["@fontsource/redaction-35"], undefined);
  ...
  assert.match(css, /raw\.githubusercontent\.com\/rektdeckard\/departure-mono\/main\/public\/assets\/DepartureMono-Regular\.woff2/);
  assert.match(css, /--font-heading:\s*"Space Grotesk Variable"/);
  assert.match(css, /--font-accent:\s*"Departure Mono"/);
  assert.match(css, /--font-sans:\s*"Inter Variable"/);
  assert.match(hero, /font-accent/);
  assert.match(hero, /text-\[0\.82em\]/);
  assert.match(hero, /font-normal/);
